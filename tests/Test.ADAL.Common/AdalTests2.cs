...
namespace Test.ADAL.Common
{
    internal partial class AdalTests
    {
        ...
        public static async Task AuthenticationParametersDiscoveryTestAsync(Sts sts)
        {
            ...
            using (Microsoft.Owin.Hosting.WebApp.Start<RelyingParty>(RelyingPartyWithDiscoveryUrl))
            {
                ...
                catch (WebException ex)
                {
                    ...
                    {
                        authParams =
                            AuthenticationParametersProxy.CreateFromResponseAuthenticateHeader(
                ...
                SetCredential(sts);
                var context = new AuthenticationContextProxy(authParams.Authority, sts.ValidateAuthority,
                    TokenCacheType.Null);
                var result =
                    await
                        context.AcquireTokenAsync(sts.ValidScope, null, sts.ValidClientId, sts.ValidDefaultRedirectUri,
                ...
                if (TestType != TestType.WinRT)
                {
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        authParams =
                            await AuthenticationParametersProxy.CreateFromUnauthorizedResponseAsync(responseMessage);
                    }

                    context = new AuthenticationContextProxy(authParams.Authority, sts.ValidateAuthority,
                        TokenCacheType.Null);
                    result =
                        await
                            context.AcquireTokenAsync(sts.ValidScope, null, sts.ValidClientId,
                                sts.ValidDefaultRedirectUri, PlatformParameters, sts.ValidUserId);
                    VerifySuccessResult(sts, result);
                }

                authParams =
                    await
                        AuthenticationParametersProxy.CreateFromResourceUrlAsync(new Uri(RelyingPartyWithDiscoveryUrl));
                context = new AuthenticationContextProxy(authParams.Authority, sts.ValidateAuthority,
                    TokenCacheType.Null);
                result =
                    await
        ...
        private static void VerifySuccessResultAndTokenContent(Sts sts, AuthenticationResultProxy result,
